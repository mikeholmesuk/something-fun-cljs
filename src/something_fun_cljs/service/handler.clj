(ns something-fun-cljs.service.handler
  (:require [something-fun-cljs.service.css :as css]
            [bidi.bidi :as bidi]
            [bidi.ring :as br]
            [bounce.core :as bc]
...
(defn page-handler [req]
  (-> (response
       (html5
        [:head
         [:title "something-fun-cljs - CLJS Single Page Web Application"]
...
(def api-routes
  ["/api" {"/echo" {:post ::echo-handler}}])

(defn api-handlers []
  {::echo-handler (-> (fn [req]
                        (response {:message (str "Hello " (:name (:body-params req)))}))
